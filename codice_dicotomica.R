...
dati = read.csv("cucina.csv", sep = ",", stringsAsFactors = F)
...
# solo in questo caso seleziono in partenza solo alcune esplicative
dati = dati[,c(1:41, 82)]
...
missing_freq = apply(dati, 2, function(col) sum(is.na(col)))
missing_freq

# righe dati mancanti
row_missing_index = which(is.na(dati))
...
# 3) Se quantitativa: potremmo provare imputazione anche se implica dei possibili errori
...
# Rimozione Leaker 
# dati$Sostenitori = NULL


# @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
# Rimozioni Osservazioni -----------------------
# @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

# seleziono un sotto insieme dei dati
# controllo valori mancanti
# sum(is.na(dati$Stato))
# 
...
# rinomimo la risposta in y
names(dati)[which(names(dati) == "is_giallo_zafferano")] = "y"

# controllo i valori della risposta
unique(dati$y)

# es modalità particolari
# dati$y = ifelse(dati$y == "valore", 1, 0)
...
# modifica della della data
# se il formato è diverso invertire i caratteri in tryFormat

# dati$Data.di.lancio = as.Date(dati$Data.di.lancio, tryFormat = "%Y-%m-%d")
# dati$Data.di.termine = as.Date(dati$Data.di.termine, tryFormat = "%Y-%m-%d")
# 
# str(dati$Data.di.lancio)
# 
...
# 1) con Format + Date
# dati$Anno = format(dati$Data.di.lancio, "%Y") # lo teniamo con categoriale
# str(dati$Anno)
# table(dati$Anno)

# 2) gsub
# pattern = "-[[:digit:]][[:digit:]]-[[:digit:]][[:digit:]]"
# head(gsub(pattern = pattern, replacement = "", x = dati$Data.di.lancio)

# 3) substring
# primi quattro caratteri (start = 1, stop = 4)
# head(substr(as.character(dati$Data.di.lancio), 1, 4))
